...
def get_config(fname):
    ...
    return conf
names = {'params': '../../final_003/q2_eq_0_mu_eq_16/params.yaml', \
         'result': '../../final_003/q2_eq_0_mu_eq_16/q1q2r_*.pkl', \
         'frames': '../../final_003/q2_eq_0_mu_eq_16/frames/q2_dump_{0:04d}'}
        ...
        for c, color in enumerate(colors):
            ...
            if arr.size > 0:
                ...
                if ind[0].size > 0:
                    ...
                    ax.scatter((arr[:, 0]), np.log10(arr[:, 2]), c=color_names[c], s=4, \
                               label=color_names[c], alpha=1.0, edgecolors='none')
    ax.scatter([1], [np.log10(2*a*tau*xs/mu/(a-xs))],c='red',s=60, marker=r'^', label="cross")
    ax.plot(np.ones(n_r), np.log10(r_grid), c='black')
